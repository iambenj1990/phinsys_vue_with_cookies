...
export const useCustomerStore = defineStore('customers', {
  ...
  actions: {
    ...
    async newCustomer(payload) {
      try {
        ...
        if (response.data.success) {
          ...
          this.customer_id = response.data.existing_name[0].id
            ? response.data.existing_name[0].id
            : '0'
        }
      } catch (error) {
    ...
    async updateCustomer(id, payload) {
      try {
        const response = await api.post('/api/customers/' + id, payload)
    ...
    async removeCustomer(payload) {
      try {
        await api.post('/api/customers/remove', payload)
      } catch (error) {
        // console.log(error)
        Notify.create({
          type: 'negative',
